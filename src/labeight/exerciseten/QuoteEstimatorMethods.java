...
public class QuoteEstimatorMethods
{
    ...
    public void addItemsToArray(int arrayPositionCounterPassedIn)
    {
        ...
        Quote myQuote= new Quote(customerName, vehicleAge, engineCC, quoteAmount);

        // Call the serialise method passing it the quote item
        mySerialiseDeserialise.serialiseTheQuoteObject(myQuote);

        // Display the current quote details for reference only
        System.out.println(myQuote.toString());

        // Call the deserialise method to retrieve the quote item
        mySerialiseDeserialise.deSerialiseTheQuoteObject();
