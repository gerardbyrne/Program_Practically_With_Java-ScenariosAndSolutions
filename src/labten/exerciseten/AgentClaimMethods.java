...
public class AgentClaimMethods
{
    ...
    public void addItemsToArray(String[] agentClaimDetails)
    {
        ...
        // Create an instance of the Claim class
        Claim myClaim = new Claim(Long.parseLong(agentClaimDetails[arrayPositionCounter]),
                                 agentClaimDetails[arrayPositionCounter + 1],
                                 agentClaimDetails[arrayPositionCounter + 2],
                                 agentClaimDetails[arrayPositionCounter + 3],
                                 Double.parseDouble(agentClaimDetails[arrayPositionCounter + 4]),
                                 Boolean.parseBoolean(agentClaimDetails[arrayPositionCounter + 5])
            );

        // Call the serialise method passing it the State item
        mySerialiseDeserialise.serialiseTheClaimObject(myClaim);

        // Display the current State details for reference only
        System.out.println(myClaim.toString());

        // Call the deserialise method to retrieve the State item
        mySerialiseDeserialise.deSerialiseTheClaimObject();
